...
public final class MusicZoneGroups {
    ...
    public static void requireValid(@Nonnull String path) {
        if (!PATH.matcher(path).matches()) {
            throw new IllegalArgumentException(
                    "Groupe invalide. Ex: spawn, spawn/donjon (2-24 caracteres par niveau, a-z 0-9 _)");
        }
        for (String segment : path.split("/")) {
            if (!SEGMENT.matcher(segment).matches()) {
                throw new IllegalArgumentException("Segment de groupe invalide: '" + segment + "'");
            }
        }
    }
