...
public final class VaryonPlayerMarkerPlugin extends JavaPlugin {

    static final String PROVIDER_KEY = "playerIcons";

    private static VaryonPlayerMarkerPlugin instance;
    private static PlayerMarkerConfig config;
    ...
    private PlayerMarkerPlayerSettingsStore playerSettingsStore;
    private PlayerMarkerAvatarService avatarService;
    private PlayerMarkerUiSounds uiSounds;
    private Path dataDirectory;

    public static VaryonPlayerMarkerPlugin getInstance() {
        return instance;
    }

    public static PlayerMarkerConfig getConfig() {
        return config;
    }
    ...
    public PlayerMarkerUiSounds getUiSounds() {
        return uiSounds;
    }

    public PlayerMarkerAvatarService getAvatarService() {
        return avatarService;
    }

    public VaryonPlayerMarkerPlugin(JavaPluginInit init) {
        super(init);
        instance = this;
    }
    ...
    protected void setup() {
        PlayerMarkerStorage.init();
        PlayerMarkerAssetPack.init();
        PlayerMarkerLiveTracker.register();
        dataDirectory = PlayerMarkerStorage.getDataRoot();
        config = PlayerMarkerConfig.load(dataDirectory);
        avatarService = new PlayerMarkerAvatarService(dataDirectory);
        playerSettingsStore = new PlayerMarkerPlayerSettingsStore(dataDirectory);
        uiSounds = new PlayerMarkerUiSounds();
        getCommandRegistry().registerCommand(new PlayerMarkerControlCommand(this));
    ...
    private void registerEventHandlers() {
        ...
        getEventRegistry().registerGlobal(PlayerReadyEvent.class, event -> {
            ...
            PlayerMarkerPlayerModelSupport.ensureRenderable(ref);
        ...
        getEventRegistry().registerGlobal(PlayerDisconnectEvent.class, event -> {
            ...
            PlayerMarkerLiveTracker.remove(uuid);
            if (avatarService != null) {
                avatarService.clearViewer(uuid);
            }
            PlayerMarkerProvider.removePersistedAvatar(uuid);
            PlayerMarkerVanishProviders.removeCacheFor(uuid);
        });
    }
    ...
    private void registerProvider(World world) {
        ...
        if (providers == null || !(providers.get(PROVIDER_KEY) instanceof PlayerMarkerProvider)) {
            installProvider(worldMapManager, PROVIDER_KEY, new PlayerMarkerProvider());
        }
    ...
    protected void shutdown() {
        ...
        PlayerMarkerLiveTracker.shutdown();
        activePlayers.clear();
    }

    public PlayerMarkerPlayerSettings resolvePlayerSettings(PlayerRef playerRef) {
        return resolvePlayerSettings(playerRef != null ? playerRef.getUuid() : null);
    }

    public PlayerMarkerPlayerSettings resolvePlayerSettings(UUID playerUuid) {
        if (playerSettingsStore == null) {
    ...
    public void applyPlayerSettings(PlayerRef playerRef, PlayerMarkerPlayerSettings settings) {
    ...
    public void openControlPage(Store<EntityStore> store, Ref<EntityStore> entityRef, PlayerRef playerRef) {
        ...
        player.getPageManager().openCustomPage(entityRef, store, new PlayerMarkerControlPage(playerRef, this));
        if (uiSounds != null) {
            uiSounds.play(playerRef, PlayerMarkerUiSounds.Cue.NAVIGATE);
        }
    }
    ...
    private void warmAvatarForPlayer(PlayerRef ref) {
        ...
        if (avatarService != null) {
            ...
            PlayerMarkerCache.getOrFetch(uuid, username);
        }
    }
