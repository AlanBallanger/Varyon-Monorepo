...
public class PayGui extends InteractiveCustomUIPage<PayGui.PayGuiData> {
    ...
    public void build(@NonNullDecl Ref<EntityStore> ref, @NonNullDecl UICommandBuilder cmd, 
        ...
        // Set initial balance
        double balance = VaryonEcotalePlugin.getInstance().getEconomyManager().getBalance(playerRef.getUuid());
        cmd.set("#BalanceValue.Text", VaryonEcotalePlugin.getInstance().getEconomyConfig().format(balance));
    ...
    private void updateFeePreview(@NonNullDecl UICommandBuilder cmd) {
        ...
        double feePercent = VaryonEcotalePlugin.getInstance().getEconomyConfig().getTransferFee();
        double fee = amount * feePercent;
        double currentBalance = VaryonEcotalePlugin.getInstance().getEconomyManager().getBalance(playerRef.getUuid());
        double balanceAfter = currentBalance - amount - fee;
        
        cmd.set("#FeeValue.Text", VaryonEcotalePlugin.getInstance().getEconomyConfig().format(fee));
        
        // Balance after: green if positive, red if negative
        if (balanceAfter >= 0) {
            cmd.set("#BalanceAfterValue.Text", VaryonEcotalePlugin.getInstance().getEconomyConfig().format(balanceAfter));
        } else {
