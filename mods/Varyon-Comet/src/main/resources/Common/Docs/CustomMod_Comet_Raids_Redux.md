...
- **/comet spawn [--tier &lt;tier&gt;] [--theme &lt;theme&gt;] [--onme true]**
  ...
  - **--tier** – Common, Rare, Epic, Legendary, or Mythic (if Endgame&QoL is enabled). Default: Common.
...
- **/comet reload**
  - Reloads **config.json** and **cometMobs.json**.
...
- **cometMobs.json** – theme definitions (monster groups): which mobs/bosses and waves per theme. Optional **spawnBlock** per theme: set to a block type ID to place that block; omit or leave blank to use the tier comet stone (e.g. `Comet_Stone_Common`). Optional **rewardOverride** per theme for custom loot per tier. The default themes set **spawnBlock** for skeleton/ghoul/undead themes to a comet coffin and for void themes to **Comet_VoidInvasion_Portal**; you can change or remove these in cometMobs.json.
...
- **tierSettings** + **rewardSettings** (in config.json)
  ...
  - **Themes** (mobs/bosses/waves), **spawnBlock**, and per-theme **rewardOverride** are all in **cometMobs.json**.
...
- **Comet_Stone_Common_Activate** – CometStoneActivateInteraction
...
**Block assets**  
Each comet stone block item (e.g. `Server/Item/Items/Comet_Stone_Common.json`) must reference the matching interaction in `BlockType.Interactions.Use`:

- `Comet_Stone_Common.json` → `"Use": "Comet_Stone_Common_Activate"`
...
Each name above must have a root interaction asset (e.g. `Server/Item/RootInteractions/Comet_Stone_Common_Activate.json`) with `"Type": "<same name>"` so the game resolves the interaction to the mod’s registered handler.
...
- **Block ID** – Comet_Stone_&lt;Tier&gt; (e.g. Comet_Stone_Common)
- **Falling projectile config** – Comet_Falling_&lt;Tier&gt; (e.g. Comet_Falling_Common in Server/ProjectileConfigs/)
- **Landing explosion particle** – Comet_Explosion_Large_&lt;Tier&gt; (e.g. Comet_Explosion_Large_Common in Server/Particles/)
- **Beam particle (on block)** – Comet_Beam_&lt;Tier&gt; (e.g. Comet_Beam_Common; referenced in block item’s BlockType.Particles)
...
- To get **F** on a no-Use block, use a block asset that includes a Use (e.g. the mod's comet stones or a custom block whose JSON has `"Use": "Comet_Stone_Common_Activate"` or the mod's shared activator).
...
- **Comet_Beam_Common** – Tier beam (green)
...
- **Tier beams** (`Comet_Beam_Common` etc.) match the comet stone tiers. **Portal beams** (`Comet_Beam_Portal_Blue`, `_Purple`, `_Cyan`) are for custom/comet portal blocks. Use one per block; the beam shows above the block and is removed when the block is broken.
...
2. **cometMobs.json** – set `spawnBlock` per theme to that block’s ID.
...
#### Step 2: Set spawnBlock in cometMobs.json

In **cometMobs.json**, each theme can have an optional **spawnBlock** field. When a comet is placed for that theme, the mod uses this block instead of the tier comet stone.
...
- **Omit or leave blank:** the mod uses the tier default (e.g. `Comet_Stone_Common`).
...
- Or a tier-specific one: **Comet_Stone_Common_Activate**, **Comet_Stone_Rare_Activate**, etc., if you want the block tied to a single tier’s handler.
...
Then set `"spawnBlock": "MyPack_Comet_Portal"` in your theme in cometMobs.json.
...
Then set `"spawnBlock": "MyPack_Comet_Coffin"` in your theme in cometMobs.json.
...
- [ ] **cometMobs.json:** theme has `"spawnBlock": "Your_Block_Id"`.
...
- **%tier%** – Comet tier name (e.g. Common, Rare, Mythic)
...
If you continue having issues, temporarily restore from a known‑good backup of **config.json**, **cometMobs.json**, then re‑apply your changes step by step.
