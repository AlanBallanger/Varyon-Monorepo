...
/**
 * Tracks comets currently placed in the world (spawn time, tier) for persistent despawn across restarts.
 * Persists to {@value #DATA_FILE_NAME}.
 */
public class CometDespawnTracker {
    
    private static final Logger LOGGER = Logger.getLogger(CometDespawnTracker.class.getName());
    private static final String DATA_FILE_NAME = "active_comets.json";
    private static final String LEGACY_DATA_FILE_NAME = "comet_despawns.json";
    ...
    private File getDataFile() {
        return resolveDataFile(DATA_FILE_NAME);
    }

    private File getLegacyDataFile() {
        return resolveDataFile(LEGACY_DATA_FILE_NAME);
    }

    private File resolveDataFile(String name) {
        try {
            File dir = com.varyon.comet.CometConfig.getConfigDirectory();
            if (dir != null) {
                if (!dir.exists()) {
                    dir.mkdirs();
                }
    ...
    private void loadFromFile(File dataFile) {
        try {
