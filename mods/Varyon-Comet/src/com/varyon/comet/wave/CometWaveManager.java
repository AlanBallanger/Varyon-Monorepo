...
public class CometWaveManager {
    ...
    /**
     * Find the registered comet block position at these coordinates (for USE when map key may differ by instance).
     */
    public Vector3i getRegisteredBlockPos(int x, int y, int z) {
        return waveState.findRegisteredPosition(x, y, z);
    }

    /** Find a registered position within distance of (x,y,z) so USE on adjacent or multi-block assets still activates. */
    ...
    /** Theme at a registered position (forced theme wins). Used when block ID alone is not enough to resolve theme. */
    ...
    private static class WaveData {
        ...
        /** Set when wave 1 spawns; used for multi-wave so theme does not depend on ConcurrentHashMap key lookup. */
        String themeId;
    ...
    public void handleCometActivation(Store<EntityStore> store, Ref<EntityStore> playerRef, Vector3i blockPos) {
        ...
        // Use canonical registered position so map lookups work (multi-block assets; allow nearby match)
        ...
        Object blockState = com.varyon.comet.loot.CometLootBlockStateUtil.getState(world, blockPos.x, blockPos.y,
                blockPos.z, false);

        if (com.varyon.comet.loot.CometLootBlockStateUtil.isItemContainerState(blockState)) {
        ...
        // Tier from block type (Comet_Stone_*) or from registration for non-stone comet blocks
        com.hypixel.hytale.server.core.asset.type.blocktype.config.BlockType blockTypeAt = null;
        try {
            blockTypeAt = world.getBlockType(blockPos.x, blockPos.y, blockPos.z);
    ...
    private void spawnWave(Store<EntityStore> store, Ref<EntityStore> playerRef, Vector3i blockPos, CometTier tier) {
        ...
        String themeId = WaveThemeProvider.selectThemeIdForWave(
                blockPos,
                tier,
                forcedThemes,
                LOGGER);
    ...
    public void updateWaveCountdown(Store<EntityStore> store, Ref<EntityStore> playerRef, WaveData waveData) {
        ...
        if (shouldUpdateTimer) {
            ...
            if (playerRefComponent != null) {
                ...
                String themeId = waveData.themeId != null ? waveData.themeId : cometThemes.get(waveData.blockPos);
                ...
                if (isBossWave) {
                    ...
                    String waveLabelTemplate = waveData.totalWaveCount > 1 ? titleTemplate : titleNoCountTemplate;
    ...
    private PlayerRef resolveFailureMessageRecipient(Store<EntityStore> store, WaveData waveData,
            java.util.UUID messageRecipientUuid) {
        try {
            com.hypixel.hytale.server.core.universe.world.World world =
                    ((com.hypixel.hytale.server.core.universe.world.storage.EntityStore) store.getExternalData())
                            .getWorld();
            if (messageRecipientUuid != null && world != null) {
                for (PlayerRef pref : world.getPlayerRefs()) {
                    if (messageRecipientUuid.equals(pref.getUuid())) {
                        return pref;
                    }
                }
            }
            if (waveData.playerRef != null && waveData.playerRef.isValid()) {
                PlayerRef pr = store.getComponent(waveData.playerRef, PlayerRef.getComponentType());
                if (pr != null) {
                    return pr;
                }
            }
        } catch (Exception e) {
            LOGGER.warning("resolveFailureMessageRecipient: " + e.getMessage());
        }
        return null;
    }

    /**
     * Shared cleanup for timeout or player death: despawn mobs, break comet, clear tracking, optional UI.
     *
     * @param failureSubtitleOverride if non-null, used as failure subtitle (player death); otherwise timeout subtitle from config
     * @param messageRecipientUuid    if non-null, resolve {@link PlayerRef} from online players for titles (respawn entity)
     */
    private void destroyCometAsFailed(Store<EntityStore> store, WaveData waveData,
        ...
        int despawned = 0;
        try {
            com.hypixel.hytale.component.CommandBuffer<EntityStore> cb = com.varyon.comet.util.CommandBufferUtil.take(store);
            if (cb != null) {
                for (Ref<EntityStore> mobRef : waveData.spawnedMobs) {
                    if (mobRef != null && mobRef.isValid()) {
                        try {
                ...
                com.varyon.comet.util.CommandBufferUtil.consume(cb);
            }
        } catch (Exception e) {
        ...
        PlayerRef playerRefComponent = resolveFailureMessageRecipient(store, waveData, messageRecipientUuid);
        if (playerRefComponent != null) {
            try {
                EventTitleUtil.hideEventTitleFromPlayer(playerRefComponent, 0.0F);
                ...
                String subtitleTemplate = (failureSubtitleOverride != null && !failureSubtitleOverride.isBlank())
                        ? failureSubtitleOverride
                        : (cfg != null ? cfg.msgWaveFailedSubtitle : "Time's Up!");
                ...
                LOGGER.warning("Error showing failed message: " + e.getMessage());
            }
        }
    }

    private void destroyCometOnTimeout(Store<EntityStore> store, WaveData waveData) {
        destroyCometAsFailed(store, waveData, null, null);
    }

    /**
     * When the participant dies during an active comet encounter, fail immediately (same cleanup as timeout, no loot).
     */
    public void handlePlayerDeathDuringComet(Store<EntityStore> store, Ref<EntityStore> entityRef) {
        if (store == null || entityRef == null || !entityRef.isValid()) {
            return;
        }
        PlayerRef playerComp = store.getComponent(entityRef, PlayerRef.getComponentType());
        if (playerComp == null) {
            return;
        }
        java.util.UUID deadUuid = playerComp.getUuid();
        if (deadUuid == null) {
            return;
        }

        List<WaveData> wavesToFail = new ArrayList<>();
        Iterator<Map.Entry<Vector3i, WaveData>> it = activeWaves.entrySet().iterator();
        while (it.hasNext()) {
            Map.Entry<Vector3i, WaveData> entry = it.next();
            Vector3i blockPos = entry.getKey();
            WaveData waveData = entry.getValue();
            if (!isCometWaveForPlayer(store, waveData, deadUuid, blockPos)) {
                continue;
            }
            it.remove();
            activeComets.remove(blockPos);
            wavesToFail.add(waveData);
        }
        if (wavesToFail.isEmpty()) {
            return;
        }

        CometConfig cfg = CometConfig.getInstance();
        String deathSubtitle = cfg != null ? cfg.msgWaveFailedPlayerDeathSubtitle : null;
        if (deathSubtitle == null || deathSubtitle.isBlank()) {
            deathSubtitle = "Vous êtes mort !";
        }

        try {
            com.hypixel.hytale.server.core.universe.world.World world =
                    ((com.hypixel.hytale.server.core.universe.world.storage.EntityStore) store.getExternalData()).getWorld();
            List<WaveData> batch = new ArrayList<>(wavesToFail);
            java.util.UUID uuidForMsg = deadUuid;
            String subtitle = deathSubtitle;
            world.execute(() -> {
                for (WaveData wd : batch) {
                    destroyCometAsFailed(store, wd, subtitle, uuidForMsg);
                }
            });
        } catch (Exception e) {
            LOGGER.warning("handlePlayerDeathDuringComet: " + e.getMessage());
        }
    }

    private boolean isCometWaveForPlayer(Store<EntityStore> store, WaveData waveData, java.util.UUID deadUuid,
            Vector3i blockPos) {
        java.util.UUID owner = cometOwners.get(blockPos);
        if (deadUuid.equals(owner)) {
            return true;
        }
        if (waveData.playerRef != null && waveData.playerRef.isValid()) {
            PlayerRef wp = store.getComponent(waveData.playerRef, PlayerRef.getComponentType());
            return wp != null && deadUuid.equals(wp.getUuid());
        }
        return false;
    }
    ...
    private void spawnBossWaveAtIndex(Store<EntityStore> store, Ref<EntityStore> playerRef,
        ...
        Vector3d centerPos = new Vector3d(blockPos.x + 0.5, blockPos.y + 1, blockPos.z + 0.5);
        final double bossRingBlocks = 2.0;
        List<Vector3d> successPositions = new ArrayList<>();
        List<String> failedBosses = new ArrayList<>();

        for (int b = 0; b < bosses.size(); b++) {
            String bossType = bosses.get(b);
            double angle = (2.0 * Math.PI * b) / bosses.size();
            Vector3d pos = new Vector3d(
                    centerPos.x + Math.cos(angle) * bossRingBlocks,
                    centerPos.y,
                    centerPos.z + Math.sin(angle) * bossRingBlocks);
            Vector3f rotation = new Vector3f(0.0f, (float) (angle + Math.PI), 0.0f);
        ...
        if (!failedBosses.isEmpty() && !successPositions.isEmpty() && world != null) {
            for (String bossType : failedBosses) {
                ...
                Vector3d retryPos = CometSpawnUtil.findValidMobSpawn(world,
                        new Vector3d(base.x + dx, base.y, base.z + dz), 11);
                if (retryPos != null) {
                    double retryAngle = Math.atan2(retryPos.x - centerPos.x, retryPos.z - centerPos.z);
                    Vector3f retryRotation = new Vector3f(0.0f, (float) (retryAngle + Math.PI), 0.0f);
                    Pair<Ref<EntityStore>, com.hypixel.hytale.server.core.universe.world.npc.INonPlayerCharacter> res =
                            spawnCometNPC(store, npcPlugin, bossType, bossType, retryPos, retryRotation, themeId, tier, zoneLevel, true);
                    if (res != null && res.first() != null) {
                        waveData.spawnedMobs.add(res.first());
                        successPositions.add(retryPos);
                    }
    ...
    private void completeWave(Store<EntityStore> store, PlayerRef playerRef, WaveData waveData) {
        ...
        if (playerRef != null) {
            ...
            CometConfig cfg = CometConfig.getInstance();
            String titleTemplate = (cfg != null ? cfg.msgWaveCompleteTitle : "Vagues vaincues !");
            String subtitleTemplate = (cfg != null ? cfg.msgWaveCompleteSubtitle : "Butin obtenu !");
            ...
            // 2. Show Loot in Chat
            String headerPrefix = (cfg != null ? cfg.msgWaveCompleteChatHeaderPrefix : "[Comète] ");
            String headerText = (cfg != null ? cfg.msgWaveCompleteChatHeader : "Vagues vaincues ! Récompenses :");
    ...
    private static void scaleRewardStacksByMultiplier(
            java.util.List<com.hypixel.hytale.server.core.inventory.ItemStack> stacks, double mult) {
        if (mult >= 1.0 - 1e-10 && mult <= 1.0 + 1e-10) {
            return;
        }
        for (int i = 0; i < stacks.size(); i++) {
            com.hypixel.hytale.server.core.inventory.ItemStack s = stacks.get(i);
            int q = s.getQuantity();
            int n = (int) Math.round(q * mult);
            if (n < 0) {
                n = 0;
            }
            if (n == 0) {
                ...
                stacks.set(i, new com.hypixel.hytale.server.core.inventory.ItemStack(s.getItemId(), n));
            }
        }
    }

    private static java.util.List<String> itemStacksToRewardLines(
            java.util.List<com.hypixel.hytale.server.core.inventory.ItemStack> stacks) {
        java.util.List<String> lines = new java.util.ArrayList<>();
        for (com.hypixel.hytale.server.core.inventory.ItemStack s : stacks) {
            lines.add(s.getQuantity() + " " + s.getItemId());
        }
        return lines;
    }
    ...
    private java.util.List<String> dropRewardsAndBreakBlock(Store<EntityStore> store, Vector3i blockPos,
        ...
        try {
            ...
            CometConfig shardCfg = CometConfig.getInstance();
            int shardCount = shardCfg != null ? shardCfg.rollShardCountForVaryonRing(zoneId, RANDOM) : 1;
            if (shardCount < 1) {
                shardCount = 1;
            }
            allItems.add(new com.hypixel.hytale.server.core.inventory.ItemStack(shardId, shardCount));

            if (shardCfg != null) {
                String mineralId = shardCfg.rollOptionalMineralBonusItem(zoneId, RANDOM);
                if (mineralId != null && !mineralId.isBlank()) {
                    allItems.add(new com.hypixel.hytale.server.core.inventory.ItemStack(mineralId, 1));
                }
            }

            CometConfig speedCfg = CometConfig.getInstance();
            double speedMult = speedCfg != null
            ...
            droppedItemIds.addAll(itemStacksToRewardLines(allItems));
