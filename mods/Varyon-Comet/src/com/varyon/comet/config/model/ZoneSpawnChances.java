...
/**
 * Tier spawn probabilities for one Hytale world-map zone (config keys {@code "1"}–{@code "4"}).
...
public class ZoneSpawnChances {
    ...
    public static Map<String, ZoneSpawnChances> generateDefaults() {
        ...
        defaults.put("1", new ZoneSpawnChances(0.7, 0.2, 0.1, 0.0, 0.0));
        defaults.put("2", new ZoneSpawnChances(0.55, 0.25, 0.15, 0.05, 0.0));
        defaults.put("3", new ZoneSpawnChances(0.45, 0.25, 0.2, 0.1, 0.0));
        defaults.put("4", new ZoneSpawnChances(0.3, 0.3, 0.25, 0.15, 0.0));
        return defaults;
    }
}
