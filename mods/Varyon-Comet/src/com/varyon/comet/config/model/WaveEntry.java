...
public class WaveEntry {
     ...
     */
    public String[] getMobIdsForTier(int cometTier, Random random) {
        List<String> result = new ArrayList<>();

        for (MobEntry mob : mobs) {
            String mobId = mob.getId();
            int count = mob.rollCountForTier(cometTier, random);
     ...
     */
    public List<String> getBossIdsForTier(int cometTier, Random random) {
        List<String> result = new ArrayList<>();
        ...
        if (randomBossSelection && !bosses.isEmpty()) {
            ...
            if (!validBosses.isEmpty()) {
                BossEntry selectedBoss = validBosses.get(random.nextInt(validBosses.size()));
                int n = selectedBoss.rollSpawnCount(random);
                for (int i = 0; i < n; i++) {
                    result.add(selectedBoss.getId());
                }
            } else if (!bosses.isEmpty()) {
                BossEntry selectedBoss = bosses.get(random.nextInt(bosses.size()));
                int n = selectedBoss.rollSpawnCount(random);
                for (int i = 0; i < n; i++) {
                    result.add(selectedBoss.getId());
                }
            ...
            for (BossEntry boss : bosses) {
                int n = boss.rollSpawnCount(random);
                for (int i = 0; i < n; i++) {
