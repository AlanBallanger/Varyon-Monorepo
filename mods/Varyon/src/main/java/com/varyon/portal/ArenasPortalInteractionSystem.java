...
public class ArenasPortalInteractionSystem extends EntityEventSystem<EntityStore, UseBlockEvent.Post> {
    ...
    public ArenasPortalInteractionSystem() {
    ...
    public void handle(int index,
        ...
        if (!blockType.getId().contains(ARENAS_PORTAL_BLOCK_ID)) {
            return;
        }
        ...
        try {
            ArenasPortalUIPage page = new ArenasPortalUIPage(playerRefComp);
            player.getPageManager().openCustomPage(ref, store, page);
            LOGGER.at(Level.FINE).log("Opened ArenasPortalMenu for player " + playerId);
        } catch (Exception e) {
            LOGGER.at(Level.WARNING).log("Failed to open ArenasPortalMenu: " + e.getMessage());
        }
    }
