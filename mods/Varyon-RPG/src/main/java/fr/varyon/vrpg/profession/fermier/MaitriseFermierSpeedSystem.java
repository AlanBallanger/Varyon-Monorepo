...
public final class MaitriseFermierSpeedSystem extends EntityTickingSystem<EntityStore> {

    private static final int CHECK_INTERVAL = 20;
    private static final float SPEED_BOOST = 0.30f;
    private static final long BOOST_DURATION_MS = 30_000L;

    private final ProfessionManager professionManager;
    private final MaitriseFermierTracker maitriseTracker;
    ...
    public MaitriseFermierSpeedSystem(@Nonnull ProfessionManager professionManager,
                                      @Nonnull MaitriseFermierTracker maitriseTracker) {
    ...
    public void tick(float deltaTime, int index,
        ...
        try {
            ...
            if (acc == null || !acc.isActive(Profession.FERMIER)
                    || acc.getTalentRank(Profession.FERMIER, "bonus_0") <= 0) {
                clearBoost(uuid, chunk, index, store);
                return;
            }

            long msSincePickup = maitriseTracker.millisSinceLastPickup(uuid);
            long now = System.currentTimeMillis();

            if (msSincePickup < BOOST_DURATION_MS) {
                long newExpiry = now - msSincePickup + BOOST_DURATION_MS;
