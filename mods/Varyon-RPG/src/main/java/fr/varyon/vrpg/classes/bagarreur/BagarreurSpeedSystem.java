...
public final class BagarreurSpeedSystem extends EntityTickingSystem<EntityStore> {
    ...
    public BagarreurSpeedSystem(@Nonnull ClassManager classManager, @Nonnull BagarreurState bagarreurState) {
    ...
    public void tick(float deltaTime, int index,
        ...
        try {
            ...
            if (acc.getActiveClass() != PlayerClass.BARBARE
                    || acc.getActiveSpec(PlayerClass.BARBARE) != PlayerSpecialization.BAGARREUR) {
                clearBoost(uuid, chunk, index, store);
                return;
            }

            float speed = bagarreurState.getAdrenalineSpeed(uuid);
            if (speed > 0f) {
                Float current = appliedBoost.get(uuid);
                if (current == null || Math.abs(current - speed) > 0.001f) {
                    appliedBoost.put(uuid, speed);
                    applyBoost(speed, chunk, index, store, playerRef);
                }
            } else {
                if (appliedBoost.containsKey(uuid)) {
                    clearBoost(uuid, chunk, index, store);
                }
            }
    ...
    private void applyBoost(float boost,
