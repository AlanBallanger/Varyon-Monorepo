...
Varyon-BossArena is a Hytale server mod for configurable boss encounters, NPC-based shop access, and per-player boss loot rewards.
...
- If neither optional economy mod is present, Varyon-BossArena falls back to item currency.

### RPGLeveling Compatibility

- Varyon-BossArena auto-detects RPGLeveling at runtime. No extra setup is required.
...
- Interacting with the guard through `Use` opens the Varyon-BossArena shop page.
...
All runtime customization is stored under `mods/Varyon-BossArena/`.
...
- If `libs/map_marker.png` exists, Varyon-BossArena copies it to world map marker assets and normalizes it to `32x32`.
...
1. Crée le dossier `libs/` à la racine du projet (il est listé dans `.gitignore` : le JAR ne doit pas être versionné).
2. Copie le fichier **`HytaleServer.jar`** fourni avec le serveur Hytale (même build que celui ciblé par le mod) et nomme-le exactement `libs/HytaleServer.jar`. Sans ce fichier valide (~100 Mo), la compilation échoue : toutes les API `com.hypixel.*` viennent de ce JAR.
...
- `build/libs/Varyon-BossArena-3.5.0.jar`
