...
val modJar = tasks.register<Zip>("modJar") {
    ...
    archiveBaseName.set("Varyon-TravelingCamera")
...
val syncToDevServerMods = tasks.register<Copy>("syncToDevServerMods") {
    group = "build"
    description = "Copie le JAR vers le dossier mods du serveur de test local"
    dependsOn(modJar)
    from(modJar)
    into(file("C:/Users/alanb/AppData/Roaming/Hytale/UserData/Saves/test/mods"))
}
    ...
    doFirst {
        ...
        if (!serverJar.isFile || serverJar.length() < 1_000_000L) {
            throw GradleException(
                "Aucun HytaleServer.jar valide trouvé pour Varyon-TravelingCamera.\n" +
                    "Copie le JAR serveur dans mods/Varyon-TravelingCamera/libs/HytaleServer.jar"
            )
        }
    }
}
